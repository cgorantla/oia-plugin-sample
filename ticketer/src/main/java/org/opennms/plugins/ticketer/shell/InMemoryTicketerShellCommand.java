...
@Command(scope = "opennms-plugin", name = "inmemory-ticketer", description = "Queries/Update tickets from InMemoryTicketer")
@Service
public class InMemoryTicketerShellCommand implements Action {

    @Reference
    private TicketingPlugin ticketingPlugin;
    ...
    public Object execute() throws Exception {
        ...
        Ticket ticket = ticketingPlugin.get(ticketId);
        if (ticket == null) {
            System.out.printf("Not able to find ticket with id %s in InMemoryTicketer \n ", ticketId);
            return null;
        } else if (ticketState != null) {
            ImmutableTicket immutableTicket = ImmutableTicket.newBuilderFrom(ticket).setState(ticketState).build();
            ticketingPlugin.saveOrUpdate(immutableTicket);
            System.out.printf("Updated ticket with state : %s \n", ticketState.name());
        }
        ticket = ticketingPlugin.get(ticketId);
